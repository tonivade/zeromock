...
public class ExamplesTest {
  ...
  @Test
  public void echoQueryParam(MockHttpServer server, HttpClient client) {
    server.when(get("/echo").and(param("say"))).then(ok(queryParam("say").andThen(plain())));

    HttpResponse response = client.request(Requests.get("/echo").withParam("say", "Hello World!"));

    assertEquals("Hello World!", asString(response.body()));
  }

  @Test
  public void echoPathParam(MockHttpServer server, HttpClient client) {
    server.when(get("/echo/:message")).then(ok(pathParam(1).andThen(plain())));

    HttpResponse response = client.request(Requests.get("/echo/saysomething"));

    assertEquals("saysomething", asString(response.body()));
  }

  @Test
  public void pojoSerialization(MockHttpServer server, HttpClient client) {
